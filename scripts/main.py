...
FORCED_SELECTIONS = {"GK": ["dúbravka"], "DEF": [], "MID": ["baleba"], "FWD": []}
...
def calculate_budget_from_previous_squad(gameweek, current_players):
    ...
    try:
        ...
        if not prev_squad_ids:
            print(
                f"Using default budget: £{BUDGET:.1f}m (could not match previous squad)"
            )
            return BUDGET
...
def create_previous_gameweek_results(current_gameweek):
    ...
    try:
        ...
        if "absolute_difference" in results_df.columns:
            results_df["absolute_difference"] = results_df["absolute_difference"].round(
                1
            )
...
def evaluate_substitute_vs_transfer(
    df, prev_squad_ids, unavailable_player_ids, free_transfers
):
    """
...
def merge_past_two_seasons(current, past_seasons, weights):
    ...
    for ppg_col, games_col, reliability_col, weight in zip(
        ppg_cols, games_cols, reliability_cols, weights
    ):
        # Fill NaN values
...
def fetch_player_fixture_difficulty(first_n_gws, players, starting_gameweek):
    ...
    """
    fixtures = pd.DataFrame(get_json("https://fantasy.premierleague.com/api/fixtures/"))
...
def build_scores(players, fixture_scores, form_weight, historic_weight, diff_weight):
    """
...
def add_next_fixture(df, target_gameweek):
    ...
    for _, p in df.iterrows():
        f = fixtures[
            (fixtures["team_h"] == p["team_id"]) | (fixtures["team_a"] == p["team_id"])
        ]
        if not f.empty:
            row = f.iloc[0]
    ...
    teams = pd.DataFrame(
        get_json("https://fantasy.premierleague.com/api/bootstrap-static/")["teams"]
    )
    nf_df = nf_df.merge(
...
def get_no_transfer_squad(df, prev_squad_ids):
    ...
    if len(available_prev_players) < 15:
        print(
            f"Warning: Only {len(available_prev_players)} " "previous players available"
        )
        return pd.DataFrame()
...
def evaluate_transfer_value(
    current_squad, potential_squad, transfers_made, free_transfers
):
    """
...
def select_squad_ilp(
            ...
            for idx, row in df.iterrows():
                if row["position"] == pos and (
                    ...
                    forced_players_info.append(
                        f"{row['display_name']} ({row['position']}, " f"{row['team']})"
                    )
                    break
    ...
    # Clean the dataframe
    df = df[
        (df["status"] == "a") | (df["chance_of_playing_next_round"].fillna(100) >= 75)
    ].copy()
    ...
    if prev_squad_ids is not None and free_transfers is not None and not WILDCARD:
        ...
        print(
            f"Must keep at least {min_players_to_keep} " "players from previous squad"
        )
...
def main():
    ...
    players = merge_past_two_seasons(players, PAST_SEASONS, HISTORIC_SEASON_WEIGHTS)
    ...
    scored = build_scores(
        players, fixture_scores, FORM_WEIGHT, HISTORIC_WEIGHT, DIFFICULTY_WEIGHT
    )
    ...
    # If we have previous squad, evaluate whether transfers are worth it
        ...
        if not no_transfer_starting.empty:
            ...
            print(f"Transfers to be made: " f"{transfer_analysis['transfers_made']}")
            print(
                f"Score improvement: " f"{transfer_analysis['score_improvement']:.3f}"
            )
            print(
                f"Value per transfer: " f"{transfer_analysis['value_per_transfer']:.3f}"
            )
            print(f"Minimum threshold: " f"{transfer_analysis['min_threshold']:.3f}")
            print(
                f"Rollover value lost: "
    ...
    updated_forced_selections = update_forced_selections_from_squad(starting, bench)
    ...
    scored_next = build_scores(
        players, fixture_scores_next, FORM_WEIGHT, HISTORIC_WEIGHT, DIFFICULTY_WEIGHT
    )
